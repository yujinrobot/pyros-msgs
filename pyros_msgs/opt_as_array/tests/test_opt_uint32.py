...
generated_modules = generate_msgsrv_nspkg(
    [os.path.join(os.path.dirname(__file__), 'msg', 'test_opt_uint32_as_array.msg')],
    ns_pkg=True
)
for m in generated_modules:
    import_msgsrv(m)

test_opt_uint32_as_array = getattr(sys.modules['gen_msgs.msg._test_opt_uint32_as_array'], 'test_opt_uint32_as_array')

import pyros_msgs.opt_as_array
# patching (need to know the field name)
pyros_msgs.opt_as_array.duck_punch(test_opt_uint32_as_array, ['data'])
...
@hypothesis.given(hypothesis.strategies.lists(hypothesis.strategies.integers(min_value=0, max_value=4294967295), max_size=1))
def test_init_rosdata(data):
    """Testing that a proper data is stored as is"""
    msg = test_opt_uint32_as_array(data=data)
    assert msg.data == data


@hypothesis.given(hypothesis.strategies.integers(min_value=0, max_value=4294967295))
...
def test_init_raw(data):
    ...
    msg = test_opt_uint32_as_array(data)
    assert msg.data == [data]


def test_init_default():
    """Testing default value"""
    msg = test_opt_uint32_as_array()
    assert msg.data == []

...
pyros_msgs.opt_as_array.duck_punch(test_gen_msgs.test_opt_uint8_as_array, ['data'])
...
def test_init_rosdata(data):
    ...
    msg = test_gen_msgs.test_opt_uint8_as_array(data=data)
...
def test_init_raw(data):
    ...
    msg = test_gen_msgs.test_opt_uint8_as_array(data)
    assert msg.data == [data]


def test_init_default():
    """Testing default value"""
    msg = test_gen_msgs.test_opt_uint8_as_array()
    assert msg.data == []
...
def test_wrong_init_except(data):
    ...
    with pytest.raises(AttributeError) as cm:
        test_gen_msgs.test_opt_uint8_as_array(data)

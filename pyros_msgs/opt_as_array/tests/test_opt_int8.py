...
pyros_msgs.opt_as_array.duck_punch(gen_test_msgs.test_opt_int8_as_array, ['data'])
...
def test_init_rosdata(data):
    ...
    msg = gen_test_msgs.test_opt_int8_as_array(data=data)
...
def test_init_raw(data):
    ...
    msg = gen_test_msgs.test_opt_int8_as_array(data)
    assert msg.data == [data]


def test_init_default():
    """Testing default value"""
    msg = gen_test_msgs.test_opt_int8_as_array()
    assert msg.data == []
...
def test_wrong_init_except(data):
    ...
    with pytest.raises(AttributeError) as cm:
        gen_test_msgs.test_opt_int8_as_array(data)

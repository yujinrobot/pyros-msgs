...
generated_modules = generate_msgsrv_nspkg(
    [os.path.join(os.path.dirname(__file__), 'msg', 'test_opt_int64_as_nested.msg')],
...
import_msgsrv('gen_msgs.msg._test_opt_int64_as_nested')

test_opt_int64_as_nested = getattr(sys.modules['gen_msgs.msg._test_opt_int64_as_nested'], 'test_opt_int64_as_nested')


import pyros_msgs.opt_as_nested
# patching (need to know the field name)
pyros_msgs.opt_as_nested.duck_punch(test_opt_int64_as_nested, ['data'])
...
@hypothesis.given(hypothesis.strategies.integers(min_value=-9223372036854775808, max_value=9223372036854775807))
def test_init_rosdata(data):
    """Testing that a proper data is stored as is"""
    msg = test_opt_int64_as_nested(data=data)
...
def test_init_raw(data):
    ...
    msg = test_opt_int64_as_nested(data)
    assert msg.data == data


def test_init_default():
    """Testing default value"""
    msg = test_opt_int64_as_nested()
    assert msg.data is None


# TODO : all possible (from ros_mappings) except proper integers
@hypothesis.given(hypothesis.strategies.one_of(
    hypothesis.strategies.integers(max_value=-9223372036854775808-1),
    hypothesis.strategies.integers(min_value=9223372036854775807+1),
    hypothesis.strategies.floats(),
))
def test_wrong_init_except(data):
    """Testing we except when types do not match"""
    with pytest.raises(AttributeError) as cm:
        test_opt_int64_as_nested(data)
